...
[NOTE]
====
Undoable commands: those commands that modify the address book`s content (`add`, `a`, `delete`, `d`, `edit`, `e`, `clear` and `c`).
====
...
* `d 1` +
`u` (reverses the `d 1` command) +
`r` (reapplies the `d 1` command) +

* `delete 1` +
`redo` +
The `redo` command fails as there are no `undo` or `u` commands executed previously.

* `d 1` +
`r` +
The `r` command fails as there are no `undo` or `u` commands executed previously.

...
* Items in square brackets are optional e.g `n/NAME [t/MOD]` can be used as `n/John Doe t/friend` or as `n/John Doe`.
...
Returns `Alex Yeoh` and `Bernice Yu` with mods containing 'friends' inside
* `findMod friends colleagues` or `fm CS2101 CS2103` + +
...
e.g. `find James Jake` +
e.g. `f James Jake`
* *Find Mod* : `findMod` or `ft KEYWORD [MORE_KEYWORDS]` +
e.g. `findMod Family Friends` +
